(function (root) {
  ...
  const FINANCE_TOOLS = [
    ...
    calculatorTool('mortgage-payment', 'Mortgage payment calculator', 'Estimate principal, interest, escrow, PMI, HOA, and the total monthly housing payment.', 'home', [
      ...
      return calcResult(total, 'estimated monthly payment', `${principalInterest}+${propertyTax}+${insurance}+${pmi}+${hoa}`, [
        ...
        { label: 'Lifetime cost of loan', value: principal + (principalInterest * months - principal) + pmi * months, prefix: '$' },
  ...
  function normalizeFavoriteFolders(raw, legacyFavorites) {
    ...
    if (!source?.length) return starterFavoriteFolders(legacyFavorites);
    ...
    return folders.length ? folders : starterFavoriteFolders(legacyFavorites);
  }
  ...
  function renderToolCategories(query) {
    ...
    const matches = TOOL_CATEGORIES.map((category) => ({
      category,
      tools: search ? category.tools.filter((tool) => toolSearchText(tool, category).includes(search)) : category.tools,
    ...
    dom.toolCategories.innerHTML = `<div class="category-matrix">${groups.map((group) => {
      ...
      const { category, tools } = selected;
      return `${row}<section class="category-panel" data-category="${category.id}"><div class="category-panel-header"><strong>${escapeHtml(category.title)}</strong><span>${escapeHtml(category.description)} · ${tools.length}</span></div><div class="category-items">${toolItems(tools)}</div></section>`;
    }).join('')}</div>`;
    return count;
  }
  ...
  function renderQuickTools() {
    ...
    const cards = childFolders.map((folder) => {
      ...
      return `<article class="quick-card folder-card">
        ...
        <span class="folder-card-actions"><button type="button" data-folder-action="rename" data-folder-id="${escapeHtml(folder.id)}">Rename</button><button type="button" data-folder-action="move" data-folder-id="${escapeHtml(folder.id)}">Move</button><button class="folder-delete" type="button" data-folder-action="delete" data-folder-id="${escapeHtml(folder.id)}" aria-label="Delete ${escapeHtml(folder.name)}">×</button></span>
      </article>`;
    });
    toolIds.forEach((id) => {
      const tool = TOOL_MAP.get(id); const category = CATEGORY_BY_TOOL.get(id);
  ...
  function createFavoriteFolder() {
    ...
    const name = root.prompt(`Name the new folder${parent ? ` inside ${parent.name}` : ''}:`, suggested);
    if (name === null) return;
    const clean = name.trim().slice(0, 48);
    if (!clean) { showToast('Folder name cannot be empty'); return; }
    state.favoriteFolders.push({ id: favoriteFolderId(), name: clean, parentId: parent?.id ?? null, toolIds: [] });
    saveFavoriteFolders(); renderQuickTools(); showToast('Folder created');
  }

  function renameFavoriteFolder(id) {
    const folder = favoriteFolderById(id); if (!folder) return;
    const name = root.prompt('Rename folder:', folder.name); if (name === null) return;
    const clean = name.trim().slice(0, 48); if (!clean) { showToast('Folder name cannot be empty'); return; }
    folder.name = clean; saveFavoriteFolders(); renderQuickTools(); showToast('Folder renamed');
  }

  function moveFavoriteFolder(id) {
    const folder = favoriteFolderById(id); if (!folder) return;
    const excluded = favoriteFolderDescendants(id); excluded.add(id);
    const choices = [{ id: null, name: 'Favorite folders (top level)' }, ...state.favoriteFolders.filter((item) => !excluded.has(item.id)).map((item) => ({ id: item.id, name: favoriteFolderPath(item.id).map((part) => part.name).join(' / ') }))];
    const currentIndex = Math.max(0, choices.findIndex((choice) => choice.id === folder.parentId));
    const answer = root.prompt(`Move “${folder.name}” into:\n${choices.map((choice, index) => `${index}. ${choice.name}`).join('\n')}`, String(currentIndex));
    if (answer === null) return;
    const index = Number(answer); if (!Number.isInteger(index) || !choices[index]) { showToast('Choose a listed folder number'); return; }
    folder.parentId = choices[index].id; saveFavoriteFolders(); renderQuickTools(); showToast('Folder moved');
  }
  ...
  function deleteFavoriteFolder(id) {
    ...
    if (!root.confirm(`Delete “${folder.name}”${nestedCount || toolCount ? ` and its ${nestedCount} nested folder${nestedCount === 1 ? '' : 's'} / ${toolCount} saved tool${toolCount === 1 ? '' : 's'}` : ''}?`)) return;
    ...
    saveFavoriteFolders(); renderQuickTools(); showToast('Folder deleted');
  }
  ...
  function geometryVisualSvg(tool, result, side = referenceSide(tool.id)) {
    ...
    if (id === 'calc-circle') {
      ...
      triangleData = { left, right, top, angles: [A, B, C] };
      ...
      if (id === 'calc-sphere') {
        ...
        shapeMarkup = (stroke) => id === 'calc-cylinder'
          ? `<path d="M0 ${radius / 2}V${height + radius / 2}Q${radius} ${height + radius} ${radius * 2} ${height + radius / 2}V${radius / 2}Z" fill="url(#shapeGreen)" stroke="${VISUAL_GREEN_DARK}" stroke-width="${stroke}" vector-effect="non-scaling-stroke"/><ellipse cx="${radius}" cy="${radius / 2}" rx="${radius}" ry="${radius / 2}" fill="#79dca4" stroke="${VISUAL_GREEN_DARK}" stroke-width="${stroke}" vector-effect="non-scaling-stroke"/>`
          : `<path d="M${radius} 0L0 ${height + radius / 2}Q${radius} ${height + radius} ${radius * 2} ${height + radius / 2}Z" fill="url(#shapeGreen)" stroke="${VISUAL_GREEN_DARK}" stroke-width="${stroke}" vector-effect="non-scaling-stroke"/>`;
      } else {
    ...
    if (triangleData) {
      ...
      const l = screen(triangleData.left); const r = screen(triangleData.right); const t = screen(triangleData.top); const [A, B, C] = triangleData.angles;
      const label = (text, x, y, anchor = 'middle') => `<text x="${x}" y="${y}" text-anchor="${anchor}" dominant-baseline="middle" fill="var(--ink-2)" stroke="var(--surface)" stroke-width="3.2" paint-order="stroke" stroke-linejoin="round" font-family="ui-sans-serif,system-ui" font-size="10" font-weight="520">${escapeHtml(text)}</text>`;
      triangleAnnotations = `<g class="triangle-annotations" pointer-events="none">${label('Leg 3 (c)', (l.x + r.x) / 2, l.y + 15)}${label('Leg 2 (b)', (l.x + t.x) / 2 - 12, (l.y + t.y) / 2, 'end')}${label('Leg 1 (a)', (r.x + t.x) / 2 + 12, (r.y + t.y) / 2, 'start')}${label(`∠1 A · ${trimNumber(A, 2)}°`, l.x + 7, l.y - 12, 'start')}${label(`∠2 B · ${trimNumber(B, 2)}°`, r.x - 7, r.y - 12, 'end')}${label(`∠3 C · ${trimNumber(C, 2)}°`, t.x, t.y + 15)}</g>`;
    }
  ...
  function quadraticGraphSvg(meta, requestedResolution) {
    ...
    const width = 820; const height = 560; const pad = 62; const vertex = -b / (2 * a); const vertexY = a * vertex * vertex + b * vertex + c;
    let xMin = Math.floor((vertex - 10) / resolution) * resolution; let xMax = Math.ceil((vertex + 10) / resolution) * resolution;
    const ySpan = Math.max(12, Math.min(240, Math.abs(a) * 60 + Math.abs(b) * 4 + 12));
    let yMin = Math.floor((vertexY - ySpan * .38) / resolution) * resolution; let yMax = Math.ceil((vertexY + ySpan * .62) / resolution) * resolution;
    if (!(xMax > xMin)) { xMin = vertex - resolution * 5; xMax = vertex + resolution * 5; }
    if (!(yMax > yMin)) { yMin = vertexY - resolution * 5; yMax = vertexY + resolution * 5; }
    ...
    const xCount = Math.max(1, Math.ceil((xMax - xMin) / resolution)); const yCount = Math.max(1, Math.ceil((yMax - yMin) / resolution));
    const xStride = Math.max(1, Math.ceil(xCount / 20)); const yStride = Math.max(1, Math.ceil(yCount / 16));
    const safeGridXStride = Math.max(1, Math.ceil(xCount / 4000)); const safeGridYStride = Math.max(1, Math.ceil(yCount / 4000));
    const labelPrecision = Math.min(10, Math.max(2, Math.ceil(-Math.log10(resolution)) + 2));
    let gridLines = ''; let labels = '';
    for (let step = 0; step <= xCount; step += safeGridXStride) {
      const x = xMin + step * resolution; if (x > xMax + resolution * .01) break;
    ...
    for (let step = 0; step <= yCount; step += safeGridYStride) {
      const y = yMin + step * resolution; if (y > yMax + resolution * .01) break;
    ...
    for (let step = 0; step <= xCount; step += xStride) {
      const x = xMin + step * resolution; if (x > xMax + resolution * .01) break;
    ...
    for (let step = 0; step <= yCount; step += yStride) {
      const y = yMin + step * resolution; if (y > yMax + resolution * .01 || Math.abs(y) < resolution * .001) continue;
  ...
  function elementVisualSvg(tool, result, side = referenceSide(tool.id)) {
    ...
    if (shape === 'cube' || shape === 'box') {
      ...
      shapeWidth = radius * 2; shapeHeight = height + radius;
      shapeMarkup = (stroke) => shape === 'cylinder'
        ? `<path d="M0 ${radius / 2}V${height + radius / 2}Q${radius} ${height + radius} ${radius * 2} ${height + radius / 2}V${radius / 2}Z" fill="url(#elementMaterial)" stroke="${dark}" stroke-width="${stroke}" vector-effect="non-scaling-stroke"/><ellipse cx="${radius}" cy="${radius / 2}" rx="${radius}" ry="${radius / 2}" fill="${light}" stroke="${dark}" stroke-width="${stroke}" vector-effect="non-scaling-stroke"/>`
        : `<path d="M${radius} 0L0 ${height + radius / 2}Q${radius} ${height + radius} ${radius * 2} ${height + radius / 2}Z" fill="url(#elementMaterial)" stroke="${dark}" stroke-width="${stroke}" vector-effect="non-scaling-stroke"/>`;
