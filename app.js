(function (root) {
  ...
  function renderQuickTools() {
    ...
    const cards = childFolders.map((folder) => {
      ...
      return `<article class="quick-card folder-card">
        ...
        <span class="folder-card-actions"><button type="button" data-folder-action="rename" data-folder-id="${escapeHtml(folder.id)}">Rename</button><button type="button" data-folder-action="move" data-folder-id="${escapeHtml(folder.id)}">Move</button></span>
  ...
  function circularSolidProjection(radius, height, type, fill, topFill, strokeColor) {
    ...
    const hiddenEdge = `<path d="M0 ${bottomCenterY}Q${centerX} ${bottomCenterY - ellipseHeight / 2} ${width} ${bottomCenterY}" fill="none" stroke="${strokeColor}" stroke-opacity=".48" stroke-width="1.7" stroke-dasharray="5 4" vector-effect="non-scaling-stroke"/>`;
    const markup = (stroke) => type === 'cylinder'
      ? `<path d="M0 ${topCenterY}V${bottomCenterY}Q${centerX} ${projectedHeight} ${width} ${bottomCenterY}V${topCenterY}Z" fill="${fill}" stroke="${strokeColor}" stroke-width="${stroke}" vector-effect="non-scaling-stroke"/><ellipse cx="${centerX}" cy="${topCenterY}" rx="${centerX}" ry="${ellipseHeight / 2}" fill="${topFill}" stroke="${strokeColor}" stroke-width="${stroke}" vector-effect="non-scaling-stroke"/>${hiddenEdge}`
      : `<path d="M${centerX} 0L0 ${bottomCenterY}Q${centerX} ${projectedHeight} ${width} ${bottomCenterY}Z" fill="${fill}" stroke="${strokeColor}" stroke-width="${stroke}" vector-effect="non-scaling-stroke"/>${hiddenEdge}`;
  ...
  function geometryVisualSvg(tool, result, side = referenceSide(tool.id)) {
    ...
    if (triangleData) {
      ...
      triangleAnnotations = `<g class="triangle-annotations" pointer-events="none">${label(`Leg 3 (c) · ${trimNumber(c, 4)}`, (l.x + r.x) / 2, l.y + 15)}${label(`Leg 2 (b) · ${trimNumber(b, 4)}`, (l.x + t.x) / 2 - 12, (l.y + t.y) / 2, 'end')}${label(`Leg 1 (a) · ${trimNumber(a, 4)}`, (r.x + t.x) / 2 + 12, (r.y + t.y) / 2, 'start')}${label(`∠1 A · ${trimNumber(A, 2)}°`, l.x + 7, l.y - 12, 'start')}${label(`∠2 B · ${trimNumber(B, 2)}°`, r.x - 7, r.y - 12, 'end')}${label(`∠3 C · ${trimNumber(C, 2)}°`, t.x, t.y + 15)}</g>`;
    }
  ...
  function bindGlobalEvents() {
    ...
    ['settingsDialog', 'historyDialog', 'infoDialog', 'folderMoveDialog'].forEach((key) => dom[key].addEventListener('click', (event) => {
