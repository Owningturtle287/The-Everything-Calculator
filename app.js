(function (root) {
  ...
  const CALCULATOR_BADGES = Object.freeze({
    ...
    'calc-circle': 'πr²', 'calc-rectangle': 'l×w×h', 'calc-triangle': '△ABC', 'calc-trapezoid': '(a+b)h/2',
  ...
  function referenceSide(toolId) {
    return Math.pow(10, Number(state.referenceExponents[toolId] ?? 0));
  }
  ...
  function referenceCube() {
    return `<g transform="translate(15 150)"><path d="M4 13 19 4l15 8-15 9Z" fill="#ef7373" stroke="#9d2222"/><path d="m4 13 15 8v18L4 30Z" fill="#d94444" stroke="#9d2222"/><path d="m19 21 15-9v18l-15 9Z" fill="#b92d2d" stroke="#9d2222"/><text x="19" y="28" text-anchor="middle" fill="white" font-size="7" font-weight="400">1³</text></g>`;
  }
  ...
  function geometryVisualSvg(tool, result, side = referenceSide(tool.id)) {
    ...
    const canvasWidth = 420; const canvasHeight = 280; const padding = 18;
    const is3d = ['calc-sphere', 'calc-cylinder', 'calc-cone', 'calc-rectangular-prism'].includes(id);
    let shapeWidth; let shapeHeight; let shapeMarkup; let referenceWidth = side; let referenceHeight = side; let referenceMarkup;
    if (id === 'calc-circle') {
      ...
      const points = `${-minX},${rawY} ${c - minX},${rawY} ${rawX - minX},0`;
      ...
      const boxMarkup = (data, stroke, colors) => {
        const p = data.points; const shift = (point) => ({ x: point.x - data.bounds.minX, y: point.y - data.bounds.minY });
        const top = [p.z, p.xz, p.xyz, p.yz].map(shift); const left = [p.o, p.z, p.yz, p.y].map(shift); const right = [p.o, p.x, p.xyz, p.z].map(shift);
        return `<polygon points="${polygonPoints(top)}" fill="${colors[0]}" stroke="${colors[3]}" stroke-width="${stroke}" vector-effect="non-scaling-stroke"/><polygon points="${polygonPoints(left)}" fill="${colors[1]}" stroke="${colors[3]}" stroke-width="${stroke}" vector-effect="non-scaling-stroke"/><polygon points="${polygonPoints(right)}" fill="${colors[2]}" stroke="${colors[3]}" stroke-width="${stroke}" vector-effect="non-scaling-stroke"/>`;
      };
      referenceMarkup = (stroke) => boxMarkup(refBox, stroke, ['#ef7373', '#d94444', '#b92d2d', '#9d2222']);
      if (id === 'calc-sphere') {
        ...
        const radius = Math.max(.000001, Number(values.radius)); const height = Math.max(.000001, Number(values.height)); shapeWidth = radius * 2; shapeHeight = height + radius;
        shapeMarkup = (stroke) => id === 'calc-cylinder'
          ? `<path d="M0 ${radius / 2}V${height + radius / 2}c0 ${radius / 2} ${radius * 2} ${radius / 2} ${radius * 2} 0V${radius / 2}" fill="url(#shapeGreen)" stroke="${VISUAL_GREEN_DARK}" stroke-width="${stroke}" vector-effect="non-scaling-stroke"/><ellipse cx="${radius}" cy="${radius / 2}" rx="${radius}" ry="${radius / 2}" fill="#79dca4" stroke="${VISUAL_GREEN_DARK}" stroke-width="${stroke}" vector-effect="non-scaling-stroke"/><ellipse cx="${radius}" cy="${height + radius / 2}" rx="${radius}" ry="${radius / 2}" fill="#238354" stroke="${VISUAL_GREEN_DARK}" stroke-width="${stroke}" vector-effect="non-scaling-stroke"/>`
          : `<path d="M${radius} 0 0 ${height + radius / 2}c0 ${radius / 2} ${radius * 2} ${radius / 2} ${radius * 2} 0Z" fill="url(#shapeGreen)" stroke="${VISUAL_GREEN_DARK}" stroke-width="${stroke}" vector-effect="non-scaling-stroke"/><ellipse cx="${radius}" cy="${height + radius / 2}" rx="${radius}" ry="${radius / 2}" fill="#238354" stroke="${VISUAL_GREEN_DARK}" stroke-width="${stroke}" vector-effect="non-scaling-stroke"/>`;
      } else {
        ...
        shapeMarkup = (stroke) => boxMarkup(data, stroke, ['#7bddaa', '#258657', '#1d7048', VISUAL_GREEN_DARK]);
      }
    }
    ...
    const scale = Math.min((canvasWidth - padding * 2) / totalWidth, (canvasHeight - padding * 2) / totalHeight);
    const left = (canvasWidth - totalWidth * scale) / 2; const top = (canvasHeight - totalHeight * scale) / 2;
    ...
    const label = is3d ? `${referenceSideLabel(side)}³` : `${referenceSideLabel(side)}×${referenceSideLabel(side)}`;
    const labelX = refX + referenceWidth * scale / 2; const labelY = Math.min(canvasHeight - 4, refY + referenceHeight * scale + 13);
    return `<svg viewBox="0 0 ${canvasWidth} ${canvasHeight}" role="img" aria-label="Scale-accurate green shape and adjustable red reference"><defs><linearGradient id="shapeGreen" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#70d99f"/><stop offset="1" stop-color="${VISUAL_GREEN_DARK}"/></linearGradient><radialGradient id="sphereGreen" cx="35%" cy="28%"><stop stop-color="#b4f0ce"/><stop offset=".45" stop-color="${VISUAL_GREEN}"/><stop offset="1" stop-color="${VISUAL_GREEN_DARK}"/></radialGradient></defs><g transform="translate(${left} ${shapeY}) scale(${scale})">${shapeMarkup(worldStroke)}</g>${reference}<text x="${labelX}" y="${labelY}" text-anchor="middle" fill="${REFERENCE_RED}" font-size="9" font-weight="400">${label}</text></svg>`;
  }
  ...
  function geometryVisualMarkup(tool, result) {
    ...
    const exponent = Number(state.referenceExponents[id] ?? 0); const side = Math.pow(10, exponent); const is3d = ['calc-sphere', 'calc-cylinder', 'calc-cone', 'calc-rectangular-prism'].includes(id);
    const label = referenceSideLabel(side);
    return `<div class="live-visual geometry-visual" data-geometry-visual="${id}"><div class="visual-toolbar"><span>${is3d ? 'Scale-accurate isometric view' : 'Scale-accurate 2D view'}</span><label class="reference-control"><span>Reference side</span><input type="range" min="-2" max="2" step=".1" value="${exponent}" data-reference-scale="${id}" aria-label="Adjust red reference side"><output>${label} unit${side === 1 ? '' : 's'}</output></label></div><div class="geometry-canvas">${geometryVisualSvg(tool, result, side)}</div><div class="visual-caption"><span>Green dimensions keep their entered proportions.</span><span class="visual-reference-label">Red ${is3d ? `cube = ${label}×${label}×${label}` : `square = ${label}×${label}`} unit${side === 1 ? '' : 's'}</span></div></div>`;
  }
  ...
  function quadraticGraphMarkup(meta) {
    ...
    let major = '';
    for (let x = Math.ceil(xMin); x <= Math.floor(xMax); x += 1) major += `<line x1="${sx(x)}" y1="${pad}" x2="${sx(x)}" y2="${height - pad}" stroke="currentColor" stroke-opacity=".12"/><text x="${sx(x)}" y="${height - 14}" text-anchor="middle" fill="currentColor" font-size="8" font-weight="400">${x}</text>`;
    for (let y = Math.ceil(yMin / yStep) * yStep; y <= yMax; y += yStep) major += `<line x1="${pad}" y1="${sy(y)}" x2="${width - pad}" y2="${sy(y)}" stroke="currentColor" stroke-opacity=".12"/><text x="${pad - 7}" y="${sy(y) + 3}" text-anchor="end" fill="currentColor" font-size="8" font-weight="400">${y}</text>`;
    ...
    const vertexY = a * vertex * vertex + b * vertex + c;
    const apex = vertexY >= yMin && vertexY <= yMax ? `<circle cx="${sx(vertex)}" cy="${sy(vertexY)}" r="4" fill="${REFERENCE_RED}" stroke="white" stroke-width="1.2"/>` : '';
    const svg = `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Zoomable live graph of the quadratic equation"><defs><pattern id="minorDots" width="${xScale / 5}" height="${Math.abs(yScale) / 5}" patternUnits="userSpaceOnUse"><circle cx=".6" cy=".6" r=".45" fill="currentColor" opacity=".16"/></pattern></defs><rect x="${pad}" y="${pad}" width="${width - pad * 2}" height="${height - pad * 2}" fill="url(#minorDots)"/>${major}${xAxis}${yAxis}<path d="${path}" fill="none" stroke="${VISUAL_GREEN}" stroke-width="3" stroke-linecap="round"/>${apex}</svg>`;
    const equation = `y = ${trimNumber(a, 6)}x² ${b < 0 ? '−' : '+'} ${trimNumber(Math.abs(b), 6)}x ${c < 0 ? '−' : '+'} ${trimNumber(Math.abs(c), 6)}`;
    return `<div class="live-visual quadratic-visual"><div class="graph-toolbar"><span>LIVE GRAPH · 0.2 subdivisions</span><span class="graph-zoom-controls"><button type="button" data-tool-action="graph-zoom-out" aria-label="Zoom graph out">−</button><button type="button" data-tool-action="graph-zoom-reset" aria-label="Reset graph zoom">${Math.round(state.graphZoom * 100)}%</button><button type="button" data-tool-action="graph-zoom-in" aria-label="Zoom graph in">+</button></span></div><div class="graph-viewport"><div class="graph-canvas" style="width:${state.graphZoom * 100}%">${svg}</div></div><div class="visual-caption quadratic-caption"><span>Red dot = parabola apex</span><span class="quadratic-equation">${escapeHtml(equation)}</span></div></div>`;
  }
  ...
  function handleToolAction(action) {
    if (action === 'reset') resetCurrentTool();
      ...
      state.graphZoom = action === 'graph-zoom-reset' ? 1 : clamp(state.graphZoom + (action === 'graph-zoom-in' ? .25 : -.25), 1, 4);
      refreshCurrentToolResult();
    }
  ...
  function bindGlobalEvents() {
    ...
    document.addEventListener('input', (event) => {
      const slider = event.target.closest?.('[data-reference-scale]');
      if (!slider || !state.currentTool || !state.currentToolResult) return;
      const toolId = slider.dataset.referenceScale;
      if (state.currentTool.id !== toolId) return;
      const exponent = clamp(Number(slider.value), -2, 2); state.referenceExponents[toolId] = exponent;
      const side = Math.pow(10, exponent); const label = referenceSideLabel(side);
      const frame = slider.closest('[data-geometry-visual]'); const canvas = frame?.querySelector('.geometry-canvas');
      if (canvas) canvas.innerHTML = geometryVisualSvg(state.currentTool, state.currentToolResult, side);
      const output = frame?.querySelector('.reference-control output');
      if (output) output.textContent = `${label} unit${side === 1 ? '' : 's'}`;
      const is3d = ['calc-sphere', 'calc-cylinder', 'calc-cone', 'calc-rectangular-prism'].includes(toolId);
