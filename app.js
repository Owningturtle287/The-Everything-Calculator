(function (root) {
  ...
  const FINANCE_TOOLS = [
    ...
    calculatorTool('mortgage-payment', 'Mortgage payment calculator', 'Estimate principal, interest, escrow, PMI, HOA, and the total monthly housing payment.', 'home', [
      ...
      return calcResult(total, 'estimated monthly payment', `${principalInterest}+${propertyTax}+${insurance}+${pmi}+${hoa}`, [
        ...
        { label: 'Amount financed', value: principal, prefix: '$' },
        { label: 'Total interest', value: principalInterest * months - principal, prefix: '$' },
      ], 'Estimate only. Actual escrow analyses, PMI rules, lender fees, closing costs, and changing taxes or premiums can alter the payment.', { currency: true });
    }),
  ...
  const HEALTH_TOOLS = [
    calculatorTool('bmi', 'BMI calculator', 'Calculate body mass index with metric or imperial measurements.', 'heart', [
      ...
      return calcResult(bmi, 'kg/m²', `${weight}/${height}^2`, [{ label: 'Adult screening category', value: category }, heightDetail], 'BMI is an adult population screening measure and does not diagnose health or account for body composition.');
  ...
  function solveTriangle(values) {
    const solve = (id) => String(values[`solve_${id}`]) === 'true' || String(values[id] ?? '').trim() === '';
    const read = (id, label) => solve(id) ? null : positive(values[id], label);
    ...
    const type = values.type || 'scalene';
    ...
    if (type === 'right') {
      if (angles[2] !== null && Math.abs(angles[2] - 90) > tolerance) throw new Error('A right triangle has angle C fixed at 90°.');
      angles[2] = 90;
    }
    if (type === 'isosceles') {
      if (sides[0] !== null && sides[1] !== null && Math.abs(sides[0] - sides[1]) > tolerance) throw new Error('For this isosceles layout, sides a and b are the equal legs.');
      if (sides[0] === null && sides[1] !== null) sides[0] = sides[1];
      if (sides[1] === null && sides[0] !== null) sides[1] = sides[0];
      if (angles[0] !== null && angles[1] !== null && Math.abs(angles[0] - angles[1]) > tolerance) throw new Error('Angles A and B must match for the selected isosceles layout.');
      if (angles[0] === null && angles[1] !== null) angles[0] = angles[1];
      if (angles[1] === null && angles[0] !== null) angles[1] = angles[0];
      if (angles[2] !== null && angles[0] === null) angles[0] = angles[1] = (180 - angles[2]) / 2;
      if (angles[0] !== null && angles[2] === null) angles[2] = 180 - 2 * angles[0];
    }
    ...
    if (sides.some((value) => value === null) || angles.some((value) => value === null)) throw new Error('Enter enough known measurements: use SSS, SAS, ASA/AAS, or a valid right-triangle combination.');
    ...
    return { sides, angles, area, perimeter };
  }
  ...
  const GEOMETRY_TOOLS = [
    ...
    calculatorTool('triangle', 'Standard triangle calculator', 'Solve right, isosceles, or scalene triangles from known sides and angles.', 'geometry', [
      selectField('type', 'Triangle type', 'right', [{ value: 'right', label: 'Right' }, { value: 'isosceles', label: 'Isosceles' }, { value: 'scalene', label: 'Scalene' }], '', { segmented: true, full: true }),
      numberField('sideA', 'Side a (opposite angle A)', 3, 'units', { min: 0, solveToggle: true }),
      numberField('sideB', 'Side b (opposite angle B)', 4, 'units', { min: 0, solveToggle: true }),
      numberField('sideC', 'Side c (opposite angle C)', 5, 'units', { min: 0, solveToggle: true, solveByDefault: true }),
      numberField('angleA', 'Angle A', 36.86989765, '°', { min: 0, max: 180, solveToggle: true, solveByDefault: true }),
      numberField('angleB', 'Angle B', 53.13010235, '°', { min: 0, max: 180, solveToggle: true, solveByDefault: true }),
      numberField('angleC', 'Angle C (fixed at 90° for right)', 90, '°', { min: 0, max: 180, solveToggle: true, solveByDefault: true }),
    ], (v) => {
  ...
  const MATH_TOOLS = [
    ...
    calculatorTool('quadratic', 'Quadratic equation solver', 'Solve ax² + bx + c = 0 and inspect the discriminant.', 'calculator', [
      ...
      if (d >= 0) {
        ...
        return calcResult(`${trimNumber(root1, 10)}, ${trimNumber(root2, 10)}`, 'real roots', `(-(${b})+sqrt(${d}))/(2*${a})`, [{ label: 'x₁', value: root1 }, { label: 'x₂', value: root2 }, { label: 'Discriminant', value: d }], '', { meta: { a, b, c } });
      }
      const real = -b / (2 * a); const imaginary = Math.sqrt(-d) / Math.abs(2 * a);
      return calcResult(`${trimNumber(real, 8)} ± ${trimNumber(imaginary, 8)}i`, 'complex roots', `${-b}/(2*${a})`, [{ label: 'Real part', value: real }, { label: 'Imaginary magnitude', value: imaginary }, { label: 'Discriminant', value: d }], '', { meta: { a, b, c } });
    }),
  ...
  const everydayUnitIds = [
    'unit-length', 'unit-area', 'unit-volume', 'unit-mass', 'unit-temperature', 'unit-time', 'unit-speed', 'unit-cooking', 'unit-fuel-economy',
  ];
  ...
  const TOOL_CATEGORIES = [
    { id: 'conversions', title: 'Everyday conversions', description: 'Length, mass, time & more', icon: ICONS.conversions, tools: everydayUnitIds.map((id) => UNIT_TOOL_MAP.get(id)) },
    { id: 'science-conversions', title: 'Science conversions', description: 'Engineering and laboratory units', icon: ICONS.scienceConversions, tools: scienceUnitIds.map((id) => UNIT_TOOL_MAP.get(id)) },
    { id: 'finance', title: 'Finance & money', description: 'Loans, savings, prices & business', icon: ICONS.finance, tools: FINANCE_TOOLS },
    { id: 'health', title: 'Health & fitness', description: 'General wellness estimates', icon: ICONS.health, tools: HEALTH_TOOLS },
    { id: 'geometry', title: 'Geometry', description: 'Area, volume and dimensions', icon: ICONS.geometry, tools: GEOMETRY_TOOLS },
    { id: 'math', title: 'Math & statistics', description: 'Algebra, percentages and data', icon: ICONS.math, tools: MATH_TOOLS },
    { id: 'science', title: 'Science & engineering', description: 'Physics, electricity and chemistry', icon: ICONS.science, tools: SCIENCE_TOOLS },
    { id: 'everyday', title: 'Everyday life', description: 'Trips, recipes, dates and projects', icon: ICONS.everyday, tools: EVERYDAY_TOOLS },
    { id: 'digital', title: 'Digital & computing', description: 'Data, displays and number systems', icon: ICONS.digital, tools: [...digitalUnitIds.map((id) => UNIT_TOOL_MAP.get(id)), ...DIGITAL_TOOLS] },
  ];
  ...
  const CALCULATOR_BADGES = Object.freeze({
    ...
    'calc-circle': 'πr²', 'calc-rectangle': 'ℓ×w', 'calc-triangle': '△ABC', 'calc-trapezoid': '(a+b)h/2',
    'calc-sphere': '⁴⁄₃πr³', 'calc-cylinder': 'πr²h', 'calc-cone': '⅓πr²h', 'calc-rectangular-prism': 'ℓwh', 'calc-regular-polygon': 'n-gon',
  ...
  function initialState() {
    ...
    return {
      ...
      favorites: Array.from({ length: Math.max(4, Array.isArray(savedFavorites) ? savedFavorites.length : 0) }, (_, index) => Array.isArray(savedFavorites) && TOOL_MAP.has(savedFavorites[index]) ? savedFavorites[index] : null),
      favoriteSlot: null,
      currentTool: null,
  ...
  function openDrawer(favoriteSlot) {
    state.favoriteSlot = Number.isInteger(favoriteSlot) ? favoriteSlot : null;
    state.openCategories.clear();
    dom.toolSearch.value = '';
    dom.drawerTitle.textContent = state.favoriteSlot === null ? 'Everything, organized.' : `Choose favorite ${state.favoriteSlot + 1}`;
    dom.drawerDescription.textContent = state.favoriteSlot === null ? 'Converters and calculators for daily life, work, and study.' : 'Pick any tool below. You can replace or remove it later.';
  ...
  function closeDrawer(options) {
    ...
    if (!options?.preserveFavoriteMode) state.favoriteSlot = null;
  ...
  function toolSearchText(tool, category) {
    ...
    return `${tool.title} ${tool.description} ${category.title} ${units}`.toLowerCase();
  }

  function renderToolCategories(query) {
    const search = String(query || '').trim().toLowerCase();
    let count = 0;
    const markup = TOOL_CATEGORIES.map((category) => {
      const tools = search ? category.tools.filter((tool) => toolSearchText(tool, category).includes(search)) : category.tools;
      if (!tools.length) return '';
      count += tools.length;
      const open = search || state.openCategories.has(category.id);
      return `<section class="category-block${open ? ' is-open' : ''}" data-category="${category.id}">
        <button class="category-toggle" type="button" data-category-id="${category.id}" aria-expanded="${open}">
          <span class="tool-icon">${iconMarkup(category.icon)}</span>
          <span><strong>${escapeHtml(category.title)}</strong><small>${escapeHtml(category.description)} · ${category.tools.length}</small></span>
          ${iconMarkup('chevron')}
        </button>
        <div class="category-items"${open ? '' : ' hidden'}>
          ${tools.map((tool) => `<button class="tool-item" type="button" data-tool-id="${tool.id}"><span>${escapeHtml(tool.shortTitle)}</span><span class="tool-badge" aria-hidden="true">${escapeHtml(toolBadge(tool))}</span></button>`).join('')}
        </div>
      </section>`;
    }).join('');
    dom.toolCategories.innerHTML = markup || `<div class="no-results">${iconMarkup('search')}<strong>No tools found</strong>Try a unit name, topic, or formula such as “mile,” “loan,” or “energy.”</div>`;
    return count;
  }

  function renderQuickTools() {
    const cards = state.favorites.map((id, index) => {
      const tool = id ? TOOL_MAP.get(id) : null;
      if (!tool) {
        return `<button class="quick-card empty-favorite" type="button" data-action="pick-favorite" data-favorite-slot="${index}">
          <span class="favorite-plus" aria-hidden="true">+</span><strong>Choose favorite</strong><span>Slot ${index + 1}</span>
        </button>`;
      }
      const category = CATEGORY_BY_TOOL.get(id);
      return `<article class="quick-card favorite-card">
        <button class="favorite-open" type="button" data-tool-id="${tool.id}"><span class="tool-icon">${iconMarkup(tool.icon)}</span><strong>${escapeHtml(tool.shortTitle)}</strong><span>${escapeHtml(category.title)}</span></button>
        <button class="favorite-remove" type="button" data-action="remove-favorite" data-favorite-slot="${index}" aria-label="Remove ${escapeHtml(tool.shortTitle)} from favorites">×</button>
      </article>`;
    });
    const canAddMore = state.favorites.length > 4 || state.favorites.slice(0, 4).every(Boolean);
    if (canAddMore) {
      cards.push(`<button class="quick-card empty-favorite add-favorite-card" type="button" data-action="pick-favorite" data-favorite-slot="${state.favorites.length}"><span class="favorite-plus" aria-hidden="true">+</span><strong>Add another</strong><span>Favorite ${state.favorites.length + 1}</span></button>`);
    }
    dom.quickGrid.innerHTML = cards.join('');
  }

  function setFavorite(slot, toolId) {
    if (!Number.isInteger(slot) || slot < 0) return;
    if (toolId && TOOL_MAP.has(toolId)) {
      while (state.favorites.length <= slot) state.favorites.push(null);
      state.favorites[slot] = toolId;
    } else if (slot < 4) state.favorites[slot] = null;
    else if (slot < state.favorites.length) state.favorites.splice(slot, 1);
    while (state.favorites.length > 4 && state.favorites.at(-1) === null) state.favorites.pop();
    safeStorageWrite(STORAGE_KEYS.favorites, state.favorites);
    renderQuickTools();
  }
  ...
  function renderCalculatorTool(tool) {
    return `<div class="calculator-layout">
      <div class="tool-form-card">
        ...
        <form id="calculatorToolForm" class="field-grid" novalidate>${tool.fields.map(fieldMarkup).join('')}</form>
        <p class="tool-error" id="toolError"></p>
        <div class="tool-actions"><button class="secondary-button" type="button" data-tool-action="reset">Reset</button><span class="action-pair"><button class="primary-button" type="button" data-tool-action="copy-tool">${iconMarkup('copy')} Copy</button><button class="secondary-button save-history-button" type="button" data-tool-action="save-tool">${iconMarkup('bookmark')} Save</button></span></div>
        ${tool.note ? `<div class="formula-banner">${iconMarkup('info')}<span>${escapeHtml(tool.note)}</span></div>` : ''}
      </div>
  ...
  function openTool(toolId, initialValues) {
  ...
  function visualFrame(svg, caption, referenceLabel) {
    return `<div class="live-visual">${svg}<div class="visual-caption"><span>${escapeHtml(caption)}</span><span style="color:${REFERENCE_RED};font-weight:750">${escapeHtml(referenceLabel)}</span></div></div>`;
  }

  function referenceSquare() {
    return `<rect x="18" y="162" width="30" height="30" rx="2" fill="${REFERENCE_RED}" fill-opacity=".78" stroke="#9d2222" stroke-width="2"/><text x="33" y="180" text-anchor="middle" fill="white" font-size="8" font-weight="800">1×1</text>`;
  }

  function referenceCube() {
    return `<g transform="translate(15 150)"><path d="M4 13 19 4l15 8-15 9Z" fill="#ef7373" stroke="#9d2222"/><path d="m4 13 15 8v18L4 30Z" fill="#d94444" stroke="#9d2222"/><path d="m19 21 15-9v18l-15 9Z" fill="#b92d2d" stroke="#9d2222"/><text x="19" y="28" text-anchor="middle" fill="white" font-size="7" font-weight="800">1³</text></g>`;
  }
  ...
  function geometryVisualMarkup(tool, result) {
    ...
    const start = `<svg viewBox="0 0 360 210" role="img" aria-label="Live green shape compared with a red one-unit reference"><defs><linearGradient id="shapeGreen" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#70d99f"/><stop offset="1" stop-color="${VISUAL_GREEN_DARK}"/></linearGradient><radialGradient id="sphereGreen" cx="35%" cy="28%"><stop stop-color="#b4f0ce"/><stop offset=".45" stop-color="${VISUAL_GREEN}"/><stop offset="1" stop-color="${VISUAL_GREEN_DARK}"/></radialGradient></defs>`;
    let shape = '';
    let reference = referenceSquare();
    let dimension = 'Live 2D representation';
    if (id === 'calc-circle') {
      shape = `<circle cx="205" cy="103" r="72" fill="${VISUAL_GREEN}" fill-opacity=".82" stroke="${VISUAL_GREEN_DARK}" stroke-width="3"/><line x1="205" y1="103" x2="277" y2="103" stroke="white" stroke-width="2" stroke-dasharray="5 4"/>`;
    } else if (id === 'calc-rectangle') {
      const l = Math.max(.01, Number(values.length)); const w = Math.max(.01, Number(values.width)); const ratio = clamp(l / w, .25, 4); const width = ratio >= 1 ? 220 : 220 * ratio; const height = ratio >= 1 ? 150 / ratio : 150;
      shape = `<rect x="${205 - width / 2}" y="${105 - height / 2}" width="${width}" height="${height}" rx="4" fill="${VISUAL_GREEN}" fill-opacity=".82" stroke="${VISUAL_GREEN_DARK}" stroke-width="3"/>`;
    } else if (id === 'calc-triangle') {
      const sides = result.meta?.sides || [3, 4, 5]; const [a, b, c] = sides; const rawX = (b * b + c * c - a * a) / (2 * c); const rawY = Math.sqrt(Math.max(0, b * b - rawX * rawX)); const scale = Math.min(220 / Math.max(c, .01), 145 / Math.max(rawY, .01)); const x1 = 80; const x2 = x1 + c * scale; const x3 = x1 + rawX * scale; const y = 174; const y3 = y - rawY * scale;
      shape = `<polygon points="${x1},${y} ${x2},${y} ${x3},${y3}" fill="${VISUAL_GREEN}" fill-opacity=".82" stroke="${VISUAL_GREEN_DARK}" stroke-width="3"/><text x="${(x1 + x2) / 2}" y="195" text-anchor="middle" fill="currentColor" font-size="10">c</text>`;
    } else if (id === 'calc-trapezoid') {
      const a = Math.max(.01, Number(values.a)); const b = Math.max(.01, Number(values.b)); const maxBase = Math.max(a, b); const top = 210 * a / maxBase; const bottom = 210 * b / maxBase;
      shape = `<polygon points="${205 - top / 2},45 ${205 + top / 2},45 ${205 + bottom / 2},170 ${205 - bottom / 2},170" fill="${VISUAL_GREEN}" fill-opacity=".82" stroke="${VISUAL_GREEN_DARK}" stroke-width="3"/>`;
    } else if (id === 'calc-regular-polygon') {
      const n = Math.max(3, Math.min(40, Math.round(Number(values.sides)))); const points = Array.from({ length: n }, (_, index) => { const angle = -Math.PI / 2 + index * Math.PI * 2 / n; return `${205 + 78 * Math.cos(angle)},${105 + 78 * Math.sin(angle)}`; }).join(' ');
      shape = `<polygon points="${points}" fill="${VISUAL_GREEN}" fill-opacity=".82" stroke="${VISUAL_GREEN_DARK}" stroke-width="3"/>`;
    } else {
      reference = referenceCube(); dimension = 'Live isometric representation';
      if (id === 'calc-sphere') shape = `<ellipse cx="210" cy="176" rx="85" ry="14" fill="#173e2a" opacity=".15"/><circle cx="210" cy="100" r="74" fill="url(#sphereGreen)" stroke="${VISUAL_GREEN_DARK}" stroke-width="3"/><ellipse cx="210" cy="100" rx="74" ry="21" fill="none" stroke="#d4f5e2" stroke-opacity=".65" stroke-width="2"/>`;
      else if (id === 'calc-cylinder') shape = `<ellipse cx="210" cy="45" rx="72" ry="22" fill="#79dca4" stroke="${VISUAL_GREEN_DARK}" stroke-width="3"/><path d="M138 45v113c0 13 32 23 72 23s72-10 72-23V45" fill="url(#shapeGreen)" stroke="${VISUAL_GREEN_DARK}" stroke-width="3"/><ellipse cx="210" cy="158" rx="72" ry="23" fill="#238354" stroke="${VISUAL_GREEN_DARK}" stroke-width="3"/>`;
      else if (id === 'calc-cone') shape = `<ellipse cx="210" cy="164" rx="82" ry="25" fill="#238354" stroke="${VISUAL_GREEN_DARK}" stroke-width="3"/><path d="M210 28 128 164c0 14 37 25 82 25s82-11 82-25Z" fill="url(#shapeGreen)" fill-opacity=".9" stroke="${VISUAL_GREEN_DARK}" stroke-width="3"/>`;
      else shape = `<g><path d="m115 72 95-45 78 36-96 47Z" fill="#7bddaa" stroke="${VISUAL_GREEN_DARK}" stroke-width="3"/><path d="m115 72 77 38v79l-77-39Z" fill="#258657" stroke="${VISUAL_GREEN_DARK}" stroke-width="3"/><path d="m192 110 96-47v79l-96 47Z" fill="#1d7048" stroke="${VISUAL_GREEN_DARK}" stroke-width="3"/></g>`;
    }
    return visualFrame(`${start}${shape}${reference}</svg>`, dimension, id.includes('sphere') || id.includes('cylinder') || id.includes('cone') || id.includes('prism') ? 'Red cube = 1×1×1 unit' : 'Red square = 1×1 unit');
  }

  function quadraticGraphMarkup(meta) {
    if (!meta) return '';
    const { a, b, c } = meta; const width = 360; const height = 240; const pad = 36; const vertex = -b / (2 * a); const xMin = Math.floor(vertex - 5); const xMax = xMin + 10;
    const samples = Array.from({ length: 241 }, (_, index) => { const x = xMin + (xMax - xMin) * index / 240; return { x, y: a * x * x + b * x + c }; });
    const sampleY = samples.map((point) => point.y); let yMin = Math.floor(Math.min(...sampleY)); let yMax = Math.ceil(Math.max(...sampleY)); if (yMin === yMax) { yMin -= 1; yMax += 1; }
    const xScale = (width - pad * 2) / (xMax - xMin); const yScale = (height - pad * 2) / (yMax - yMin); const sx = (x) => pad + (x - xMin) * xScale; const sy = (y) => height - pad - (y - yMin) * yScale;
    const yStep = Math.max(1, Math.ceil((yMax - yMin) / 9));
    let major = '';
    for (let x = Math.ceil(xMin); x <= Math.floor(xMax); x += 1) major += `<line x1="${sx(x)}" y1="${pad}" x2="${sx(x)}" y2="${height - pad}" stroke="currentColor" stroke-opacity=".12"/><text x="${sx(x)}" y="${height - 14}" text-anchor="middle" fill="currentColor" font-size="8">${x}</text>`;
    for (let y = Math.ceil(yMin / yStep) * yStep; y <= yMax; y += yStep) major += `<line x1="${pad}" y1="${sy(y)}" x2="${width - pad}" y2="${sy(y)}" stroke="currentColor" stroke-opacity=".12"/><text x="${pad - 7}" y="${sy(y) + 3}" text-anchor="end" fill="currentColor" font-size="8">${y}</text>`;
    const path = samples.map((point, index) => `${index ? 'L' : 'M'}${sx(point.x).toFixed(2)} ${sy(point.y).toFixed(2)}`).join(' ');
    const xAxis = yMin <= 0 && yMax >= 0 ? `<line x1="${pad}" y1="${sy(0)}" x2="${width - pad}" y2="${sy(0)}" stroke="currentColor" stroke-width="1.4"/>` : '';
    const yAxis = xMin <= 0 && xMax >= 0 ? `<line x1="${sx(0)}" y1="${pad}" x2="${sx(0)}" y2="${height - pad}" stroke="currentColor" stroke-width="1.4"/>` : '';
    const svg = `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Live graph of the quadratic equation"><defs><pattern id="minorDots" width="${xScale / 10}" height="${Math.abs(yScale) / 10}" patternUnits="userSpaceOnUse"><circle cx=".6" cy=".6" r=".45" fill="currentColor" opacity=".16"/></pattern></defs><rect x="${pad}" y="${pad}" width="${width - pad * 2}" height="${height - pad * 2}" fill="url(#minorDots)"/>${major}${xAxis}${yAxis}<path d="${path}" fill="none" stroke="${VISUAL_GREEN}" stroke-width="3" stroke-linecap="round"/></svg>`;
    return visualFrame(svg, 'Whole-number axes with 0.1-unit minor dots', `y = ${trimNumber(a, 6)}x² ${b < 0 ? '−' : '+'} ${trimNumber(Math.abs(b), 6)}x ${c < 0 ? '−' : '+'} ${trimNumber(Math.abs(c), 6)}`);
  }
  ...
  function elementVisualMarkup(tool, result) {
    ...
    const [light, dark] = elementAppearance(result.meta.element, result.meta.phase); const shape = result.meta.shape;
    const defs = `<defs><linearGradient id="material" x1="0" y1="0" x2="1" y2="1"><stop stop-color="${light}"/><stop offset=".58" stop-color="${dark}"/><stop offset=".82" stop-color="${light}"/><stop offset="1" stop-color="${dark}"/></linearGradient><radialGradient id="materialSphere" cx="32%" cy="26%"><stop stop-color="#fff" stop-opacity=".9"/><stop offset=".25" stop-color="${light}"/><stop offset="1" stop-color="${dark}"/></radialGradient><pattern id="materialGrain" width="9" height="9" patternUnits="userSpaceOnUse"><circle cx="2" cy="3" r=".7" fill="#fff" opacity=".24"/><circle cx="7" cy="6" r=".6" fill="#111" opacity=".16"/></pattern></defs>`;
    let graphic;
    if (shape === 'sphere' || shape === 'ellipsoid') graphic = `<ellipse cx="210" cy="104" rx="${shape === 'sphere' ? 75 : 98}" ry="${shape === 'sphere' ? 75 : 58}" fill="url(#materialSphere)" stroke="${dark}" stroke-width="3"/><ellipse cx="210" cy="104" rx="${shape === 'sphere' ? 75 : 98}" ry="${shape === 'sphere' ? 75 : 58}" fill="url(#materialGrain)"/>`;
    else if (shape === 'cylinder') graphic = `<ellipse cx="210" cy="45" rx="72" ry="22" fill="${light}" stroke="${dark}" stroke-width="3"/><path d="M138 45v113c0 13 32 23 72 23s72-10 72-23V45" fill="url(#material)" stroke="${dark}" stroke-width="3"/><ellipse cx="210" cy="158" rx="72" ry="23" fill="${dark}" stroke="${dark}" stroke-width="3"/>`;
    else if (shape === 'cone') graphic = `<ellipse cx="210" cy="164" rx="82" ry="25" fill="${dark}" stroke="${dark}" stroke-width="3"/><path d="M210 28 128 164c0 14 37 25 82 25s82-11 82-25Z" fill="url(#material)" stroke="${dark}" stroke-width="3"/>`;
    else if (shape === 'pyramid') graphic = `<path d="m210 25-98 132 80 38Z" fill="${dark}" stroke="${dark}" stroke-width="3"/><path d="m210 25 82 123-100 47Z" fill="url(#material)" stroke="${dark}" stroke-width="3"/><path d="m112 157 100-42 80 33-100 47Z" fill="${light}" opacity=".5"/>`;
    else if (shape === 'triangular') graphic = `<path d="m115 72 58-42 115 32-58 44Z" fill="${light}" stroke="${dark}" stroke-width="3"/><path d="m115 72 58-42v116l-58 43Z" fill="${dark}" stroke="${dark}" stroke-width="3"/><path d="m115 189 58-43 115-4-58 47Z" fill="url(#material)" stroke="${dark}" stroke-width="3"/>`;
    else graphic = `<path d="m115 72 95-45 78 36-96 47Z" fill="${light}" stroke="${dark}" stroke-width="3"/><path d="m115 72 77 38v79l-77-39Z" fill="${dark}" stroke="${dark}" stroke-width="3"/><path d="m192 110 96-47v79l-96 47Z" fill="url(#material)" stroke="${dark}" stroke-width="3"/><path d="m115 72 95-45 78 36-96 47-77-38Z" fill="url(#materialGrain)"/>`;
    return visualFrame(`<svg viewBox="0 0 360 210" role="img" aria-label="Isometric ${escapeHtml(shape)} with material styling for ${escapeHtml(result.meta.element)}">${defs}${graphic}${referenceCube()}</svg>`, `${result.meta.element} reference appearance · ${shape}`, 'Red cube = 1×1×1 unit');
  }
  ...
  async function calculateCurrencyTool(tool, request) {
    ...
    try {
      ...
      if (from !== to) {
        ...
        try {
          const response = await root.fetch(`https://api.frankfurter.app/latest?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`, { cache: 'no-store', headers: { Accept: 'application/json' } });
          if (!response.ok) throw new Error(`Rate service returned ${response.status}.`);
          const payload = await response.json(); rate = Number(payload?.rates?.[to]); rateDate = String(payload?.date || 'Latest available');
  ...
  function calculateCurrentCalculatorTool(tool) {
    ...
    const resultElement = document.getElementById('toolResult');
    if (!resultElement) return;
    const missing = [...dom.toolWorkspace.querySelectorAll('[data-field-wrapper]:not([hidden]) [data-calc-field]')]
  ...
  function handleAction(action, source) {
    switch (action) {
      ...
      case 'pick-favorite': openDrawer(Number(source.dataset.favoriteSlot)); break;
      case 'remove-favorite': setFavorite(Number(source.dataset.favoriteSlot), null); showToast('Favorite removed'); break;
      case 'copy-result':
  ...
  function cacheDom() {
    dom = {
      ...
      calculatorView: document.getElementById('calculatorView'), toolView: document.getElementById('toolView'), toolWorkspace: document.getElementById('toolWorkspace'), toolPageCategory: document.getElementById('toolPageCategory'), toolPageTitle: document.getElementById('toolPageTitle'), toolPageDescription: document.getElementById('toolPageDescription'), toolPageIcon: document.getElementById('toolPageIcon'),
      quickGrid: document.getElementById('quickGrid'), toolDrawer: document.getElementById('toolDrawer'), drawerBackdrop: document.getElementById('drawerBackdrop'), toolSearch: document.getElementById('toolSearch'), toolCategories: document.getElementById('toolCategories'), drawerTitle: document.getElementById('drawerTitle'), drawerDescription: document.getElementById('drawerDescription'),
      ...
      historyDialog: document.getElementById('historyDialog'), historyList: document.getElementById('historyList'), infoDialog: document.getElementById('infoDialog'), infoEyebrow: document.getElementById('infoEyebrow'), infoTitle: document.getElementById('infoTitle'), infoContent: document.getElementById('infoContent'), toast: document.getElementById('toast'),
    };
  }
  ...
  function bindGlobalEvents() {
    ...
    document.addEventListener('click', (event) => {
      ...
      if (toolButton) {
        if (state.favoriteSlot !== null) {
          const slot = state.favoriteSlot;
          setFavorite(slot, toolButton.dataset.toolId);
          closeDrawer();
          showToast(`Favorite ${slot + 1} saved`);
        } else openTool(toolButton.dataset.toolId);
        return;
      }
      const categoryButton = event.target.closest('[data-category-id]');
      if (categoryButton) {
        const id = categoryButton.dataset.categoryId;
        if (state.openCategories.has(id)) state.openCategories.delete(id); else state.openCategories.add(id);
      ...
      if (historyButton) {
        ...
        if (item) {
          ...
          if (item.kind === 'tool' && item.toolId) openTool(item.toolId, item.values);
    ...
    ['settingsDialog', 'historyDialog', 'infoDialog'].forEach((key) => dom[key].addEventListener('click', (event) => {
