(function (root) {
  ...
  function textField(id, label, value, options) {
    const opts = options || {};
    return { id, label, type: 'text', default: value || '', placeholder: opts.placeholder || '', help: opts.help || '', full: Boolean(opts.full), when: opts.when || null };
  }
  ...
  function calcResult(primary, unitLabel, expression, details, note, options) {
    return { primary, unit: unitLabel || '', expression: expression || '', details: details || [], note: note || '', currency: Boolean(options?.currency) };
  }
  ...
  const SCIENCE_TOOLS = [
    ...
    calculatorTool('element-shape', 'Element to shape calculator', 'Estimate a shaped element’s volume, mass, and Earth weight from its density.', 'flask', [
      selectField('element', 'Element', 'Al', ELEMENTS.map((element) => ({ value: element.symbol, label: `${element.atomicNumber}. ${element.name} (${element.symbol})` })), 'All 118 elements are included.', { full: true }),
      ...
      const weightNewtons = massKg * 9.80665;
      return calcResult(massKg, 'kg mass', expression, [
        { label: 'Volume', value: volume, suffix: ` ${v.dimensionUnit}³` },
        { label: 'Volume in cm³', value: volumeCm3, suffix: ' cm³' },
        { label: 'Mass', value: massGrams, suffix: ' g' },
        { label: 'Mass', value: massKg / 0.45359237, suffix: ' lb' },
        { label: 'Earth weight', value: weightNewtons, suffix: ' N' },
        { label: 'Earth weight', value: weightNewtons / 4.4482216152605, suffix: ' lbf' },
      ...
      ], `Density varies with temperature, pressure, allotrope, and purity. Gas densities use standard-condition references; values marked predicted are theoretical because no bulk sample has been measured.`);
  ...
  const EVERYDAY_TOOLS = [
    calculatorTool('trip-fuel-cost', 'Trip fuel cost calculator', 'Estimate fuel needed and cost for a road trip.', 'home', [
      ...
      numberField('distance', 'Trip distance', 500, 'km or mi', { min: 0 }), numberField('economy', 'Consumption / economy', 8, 'L/100 km or mpg', { min: 0 }), numberField('price', 'Fuel price', 1.65, 'per L or US gal', { min: 0 }),
    ], (v) => {
      ...
      const imperial = v.system === 'imperial';
      const fuel = imperial ? distance / economy : distance * economy / 100;
      return calcResult(fuel * price, 'trip cost', imperial ? `${distance}/${economy}*${price}` : `${distance}*${economy}/100*${price}`, [
    ...
    calculatorTool('paint', 'Paint coverage calculator', 'Estimate paint volume from wall area, coverage, and coats.', 'home', [
      ...
      const area = positive(v.area, 'Area', true); const coverage = positive(v.coverage, 'Coverage'); const coats = Math.max(1, Math.round(positive(v.coats, 'Coats'))); const paint = area * coats / coverage; const imperial = v.system === 'imperial';
      return calcResult(paint, imperial ? 'US gallons of paint' : 'liters of paint', `${area}*${coats}/${coverage}`, [{ label: 'Recommended with 10% buffer', value: paint * 1.1, suffix: imperial ? ' US gal' : ' L' }, { label: 'Coated area', value: area * coats, suffix: imperial ? ' ft²' : ' m²' }]);
    }),
    ...
    calculatorTool('work-time', 'Work time calculator', 'Find elapsed paid time after an unpaid break.', 'clock', [
      ...
      textField('start', 'Start time', '8.5', { help: 'Use 8.5 in decimal mode or 08:30 in actual-time mode.' }), textField('end', 'End time', '17.25', { help: 'Overnight shifts are handled automatically.' }), numberField('break', 'Unpaid break', 30, 'minutes', { min: 0 }),
    ], (v) => {
      const parseClock = (raw, label) => {
        const match = String(raw).trim().match(/^(\d{1,2}):(\d{2})$/);
        if (!match || Number(match[1]) > 23 || Number(match[2]) > 59) throw new Error(`${label} must use HH:MM.`);
        return Number(match[1]) + Number(match[2]) / 60;
      };
      const actual = v.format === 'clock';
      const start = actual ? parseClock(v.start, 'Start time') : finiteNumber(v.start, 'Start time');
      let end = actual ? parseClock(v.end, 'End time') : finiteNumber(v.end, 'End time');
  ...
  function initialState() {
    ...
    return {
      ...
      favorites: Array.from({ length: 4 }, (_, index) => Array.isArray(savedFavorites) && TOOL_MAP.has(savedFavorites[index]) ? savedFavorites[index] : null),
      favoriteSlot: null,
      currentTool: null,
  ...
  function renderQuickTools() {
    dom.quickGrid.innerHTML = state.favorites.map((id, index) => {
  ...
  function setFavorite(slot, toolId) {
    ...
    state.favorites[slot] = toolId && TOOL_MAP.has(toolId) ? toolId : null;
    ...
    return `<div class="converter-layout">
      <div class="tool-form-card">
        ...
        <div class="field-stack">
          <label class="field-card"><span>From</span><span class="input-shell"><input id="unitInput" data-unit-value type="number" inputmode="decimal" value="" placeholder="Example: 1" step="any" aria-label="Value to convert"><select id="unitFrom" data-unit-from aria-label="From unit">${unitOptions(tool, tool.defaultFrom)}</select></span></label>
        ...
        <div class="tool-actions"><button class="secondary-button" type="button" data-tool-action="reset">Reset</button><button class="primary-button" type="button" data-tool-action="copy-tool">${iconMarkup('copy')} Copy result</button><button class="secondary-button save-history-button" type="button" data-tool-action="save-tool">${iconMarkup('bookmark')} Save to History</button></div>
  ...
  function fieldMarkup(field) {
    ...
    if (field.type === 'select' && field.segmented) {
      ...
      control = `<span class="input-shell"><input id="field-${field.id}" name="${field.id}" data-calc-field type="number" inputmode="decimal" value="" step="${escapeHtml(field.step)}"${field.min !== undefined ? ` min="${field.min}"` : ''}${field.max !== undefined ? ` max="${field.max}"` : ''} placeholder="${escapeHtml(field.placeholder || `Example: ${field.default}`)}"><span style="padding:0 .85rem;color:var(--ink-3);font-size:.72rem;white-space:nowrap">${escapeHtml(field.unit)}</span></span>`;
    } else {
      const type = field.type === 'date' ? 'date' : field.type === 'number' ? 'number' : 'text';
      const inputMode = field.type === 'number' ? ' inputmode="decimal"' : '';
      control = `<input class="smart-input" id="field-${field.id}" name="${field.id}" data-calc-field type="${type}"${inputMode} value=""${field.step ? ` step="${escapeHtml(field.step)}"` : ''}${field.min !== undefined ? ` min="${field.min}"` : ''}${field.max !== undefined ? ` max="${field.max}"` : ''} placeholder="${escapeHtml(field.placeholder || `Example: ${field.default}`)}">`;
    }
    return `<div class="${classes}" data-field-wrapper="${escapeHtml(field.id)}"${condition}><label class="input-label" for="field-${field.id}">${escapeHtml(field.label)}</label>${control}${field.help ? `<span class="field-help">${escapeHtml(field.help)}</span>` : ''}</div>`;
  }
  ...
  function calculateCurrentCalculatorTool(tool) {
    ...
    const missing = [...dom.toolWorkspace.querySelectorAll('[data-field-wrapper]:not([hidden]) [data-calc-field]')]
      .some((element) => element.type !== 'hidden' && element.tagName !== 'SELECT' && !String(element.value).trim());
    if (missing) {
      errorElement.textContent = '';
  ...
  function resetCurrentTool() {
    ...
    if (tool.kind === 'unit') {
      ...
      tool.fields.forEach((field) => { const element = document.getElementById(`field-${field.id}`); if (element) element.value = field.type === 'select' ? String(field.default) : ''; });
      syncSegmentedFields();
      updateConditionalFields();
    }
    refreshCurrentToolResult();
  ...
  function bindToolEvents() {
    dom.toolWorkspace.querySelectorAll('input, select').forEach((element) => {
      element.addEventListener('input', () => { updateConditionalFields(); refreshCurrentToolResult(); });
  ...
  function currentToolSnapshot() {
    ...
    return Object.fromEntries(tool.fields.map((field) => [field.id, document.getElementById(`field-${field.id}`)?.value || '']));
  }
  ...
  function openInfo(type) {
    ...
    if (type === 'about') {
      ...
      dom.infoContent.innerHTML = `<h3>1.0.2</h3><ul class="capability-list"><li>Top-drop toolbox and thinner header</li><li>Expanded scientific keypad, memory, 2nd functions, and local RAD/DEG control</li><li>Four user-selected favorite slots</li><li>Blank-by-default tool inputs and manual tool history</li><li>Loan down payments and full mortgage estimate</li><li>Metric/imperial and clock-format options</li><li>118-element shape, mass, and weight calculator</li><li>Money fixed to two decimals and precision choices from 1–15</li></ul><h3>1.0.0</h3><p>Initial calculator, conversion catalog, themes, and offline web app.</p>`;
    } else {
