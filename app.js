(function (root) {
  ...
  function roundTo(value, precision) {
    ...
    const p = clamp(Number(precision) || 8, 0, 14);
  ...
  function formatNumber(value, settings, options) {
    ...
    if (Object.is(value, -0)) value = 0;
    const precision = clamp(Number(opts.precision ?? settings?.precision ?? 8), 0, 14);
    const abs = Math.abs(value);
  ...
  const UNIT_TOOLS = [
    ...
    unitTool('time', 'Time converter', 'Convert from nanoseconds to calendar-scale time units.', [
      ...
      unit('Hour', 'h', 3600), unit('Day', 'day', 86400), unit('Week', 'wk', 604800), unit('Fortnight', 'fn', 1209600), unit('Julian year', 'yr', 31557600),
    ], { defaultFrom: 5, defaultTo: 4, icon: 'clock', note: 'A year uses the Julian convention of 365.25 days.' }),
    unitTool('speed', 'Speed converter', 'Convert road, marine, aviation, and scientific speed units.', [
      ...
      unit('Foot per second', 'ft/s', 0.3048), unit('Knot', 'kn', 0.514444444444), unit('Mach (standard air)', 'Mach', 343),
  ...
  function textField(id, label, value, options) {
    const opts = options || {};
    return { id, label, type: 'text', default: value || '', placeholder: opts.placeholder || '', help: opts.help || '', full: Boolean(opts.full) };
  }

  function dateField(id, label, value, options) {
    const opts = options || {};
    return { id, label, type: 'date', default: value || '', help: opts.help || '', full: Boolean(opts.full) };
  }

  function selectField(id, label, value, options, help) {
    return { id, label, type: 'select', default: value, options, help: help || '' };
  }
  ...
  function calcResult(primary, unitLabel, expression, details, note) {
    return { primary, unit: unitLabel || '', expression: expression || '', details: details || [], note: note || '' };
  }

  const FINANCE_TOOLS = [
    calculatorTool('loan-payment', 'Loan payment calculator', 'Estimate periodic payments, total paid, and interest for a fixed-rate amortizing loan.', 'wallet', [
      numberField('principal', 'Loan amount', 250000, '$', { min: 0 }),
      ...
      const principal = positive(v.principal, 'Loan amount');
      ...
      ], 'Estimate only. Fees, taxes, insurance, and lender-specific compounding are not included.');
    }),
    calculatorTool('compound-interest', 'Compound interest calculator', 'Project growth with compounding and optional monthly contributions.', 'wallet', [
      ...
      ], 'This is a mathematical projection, not an investment forecast. Returns and contribution timing vary.');
    }),
    ...
    calculatorTool('savings-goal', 'Savings goal calculator', 'Estimate the monthly deposit needed to reach a future balance.', 'wallet', [
      ...
      return calcResult(payment, 'per month', rate === 0 ? `${gap}/${months}` : `${gap}*${rate}/((1+${rate})^${months}-1)`, [
        ...
        { label: 'Months', value: months },
      ], 'Projection assumes a constant return and end-of-month deposits.');
    }),
  ...
  const HEALTH_TOOLS = [
    calculatorTool('bmi', 'BMI calculator', 'Calculate body mass index from metric height and weight.', 'heart', [
      numberField('weight', 'Weight', 72, 'kg', { min: 0 }), numberField('height', 'Height', 175, 'cm', { min: 0 }),
    ], (v) => {
      const weight = positive(v.weight, 'Weight');
      const height = positive(v.height, 'Height') / 100;
      const bmi = weight / (height * height);
      const category = bmi < 18.5 ? 'Below standard range' : bmi < 25 ? 'Standard range' : bmi < 30 ? 'Above standard range' : 'High range';
      return calcResult(bmi, 'kg/m²', `${weight}/${height}^2`, [{ label: 'Screening category', value: category }, { label: 'Height', value: height, suffix: ' m' }], 'BMI is a population screening measure and does not diagnose health or account for body composition.');
    }, { note: 'For general informational use only; discuss personal health decisions with a qualified clinician.' }),
    calculatorTool('bmr', 'BMR calculator', 'Estimate resting energy use with the Mifflin–St Jeor equation.', 'heart', [
      selectField('sex', 'Equation', 'male', [{ value: 'male', label: 'Male constant (+5)' }, { value: 'female', label: 'Female constant (−161)' }]),
      numberField('age', 'Age', 30, 'years', { min: 1 }), numberField('weight', 'Weight', 72, 'kg', { min: 0 }), numberField('height', 'Height', 175, 'cm', { min: 0 }),
    ], (v) => {
      const weight = positive(v.weight, 'Weight');
      const height = positive(v.height, 'Height');
    ...
    calculatorTool('body-surface-area', 'Body surface area calculator', 'Estimate body surface area using the Mosteller formula.', 'heart', [
      numberField('weight', 'Weight', 72, 'kg', { min: 0 }), numberField('height', 'Height', 175, 'cm', { min: 0 }),
    ], (v) => {
      const weight = positive(v.weight, 'Weight');
      const height = positive(v.height, 'Height');
    ...
    calculatorTool('pace', 'Running pace calculator', 'Convert a distance and finish time into pace and average speed.', 'heart', [
      numberField('distance', 'Distance', 5, 'km', { min: 0 }), numberField('minutes', 'Finish time', 28, 'minutes', { min: 0 }),
    ], (v) => {
      ...
      return calcResult(`${adjustedMinutes}:${String(paceSeconds === 60 ? 0 : paceSeconds).padStart(2, '0')}`, 'min/km', `${minutes}/${distance}`, [
        { label: 'Average speed', value: distance / (minutes / 60), suffix: ' km/h' }, { label: 'Raw pace', value: pace, suffix: ' min/km' },
      ]);
    }),
  ...
  const GEOMETRY_TOOLS = [
    ...
    calculatorTool('triangle', 'Triangle calculator', 'Find area from base and perpendicular height.', 'geometry', [numberField('base', 'Base', 10, 'units', { min: 0 }), numberField('height', 'Height', 7, 'units', { min: 0 })], (v) => {
      const b = positive(v.base, 'Base', true); const h = positive(v.height, 'Height', true);
      return calcResult(b * h / 2, 'square units', `${b}*${h}/2`, [{ label: 'Base', value: b, suffix: ' units' }, { label: 'Height', value: h, suffix: ' units' }]);
    }),
    ...
    calculatorTool('pythagorean', 'Right triangle calculator', 'Solve the hypotenuse from two perpendicular sides.', 'geometry', [numberField('a', 'Side a', 3, 'units', { min: 0 }), numberField('b', 'Side b', 4, 'units', { min: 0 })], (v) => {
      const a = positive(v.a, 'Side a', true); const b = positive(v.b, 'Side b', true); const c = Math.hypot(a, b);
      return calcResult(c, 'hypotenuse', `sqrt(${a}^2+${b}^2)`, [{ label: 'Area', value: a * b / 2, suffix: ' square units' }, { label: 'Perimeter', value: a + b + c, suffix: ' units' }]);
    }),
    calculatorTool('regular-polygon', 'Regular polygon calculator', 'Find area and perimeter from side count and side length.', 'geometry', [numberField('sides', 'Number of sides', 6, '', { min: 3, step: 1 }), numberField('length', 'Side length', 5, 'units', { min: 0 })], (v) => {
      ...
      return calcResult(area, 'square units', `${n}*${s}^2/(4*tan(${180 / n}))`, [{ label: 'Perimeter', value: n * s, suffix: ' units' }, { label: 'Interior angle', value: (n - 2) * 180 / n, suffix: '°' }]);
    }),
  ];
  ...
  const MATH_TOOLS = [
    ...
    calculatorTool('quadratic', 'Quadratic equation solver', 'Solve ax² + bx + c = 0 and inspect the discriminant.', 'calculator', [
      ...
      if (d >= 0) {
        ...
        return calcResult(`${trimNumber(root1, 10)}, ${trimNumber(root2, 10)}`, 'real roots', `(-(${b})+sqrt(${d}))/(2*${a})`, [{ label: 'x₁', value: root1 }, { label: 'x₂', value: root2 }, { label: 'Discriminant', value: d }]);
      }
      const real = -b / (2 * a); const imaginary = Math.sqrt(-d) / Math.abs(2 * a);
      return calcResult(`${trimNumber(real, 8)} ± ${trimNumber(imaginary, 8)}i`, 'complex roots', `${-b}/(2*${a})`, [{ label: 'Real part', value: real }, { label: 'Imaginary magnitude', value: imaginary }, { label: 'Discriminant', value: d }]);
    }),
    calculatorTool('statistics', 'Mean, median & mode', 'Summarize a comma-separated list of numeric observations.', 'calculator', [
      textField('values', 'Values', '4, 7, 7, 9, 13, 15', { full: true, placeholder: '1, 2, 3, 4' }),
    ], (v) => {
    ...
    calculatorTool('standard-deviation', 'Standard deviation calculator', 'Calculate population or sample spread for a list of values.', 'calculator', [
      textField('values', 'Values', '10, 12, 13, 16, 19', { full: true }),
  ...
  const EVERYDAY_TOOLS = [
    calculatorTool('trip-fuel-cost', 'Trip fuel cost calculator', 'Estimate fuel needed and cost for a road trip.', 'home', [
      numberField('distance', 'Trip distance', 500, 'km', { min: 0 }), numberField('economy', 'Fuel consumption', 8, 'L/100 km', { min: 0 }), numberField('price', 'Fuel price', 1.65, 'per L', { min: 0 }),
    ], (v) => {
      const distance = positive(v.distance, 'Trip distance', true); const economy = positive(v.economy, 'Fuel consumption', true); const price = positive(v.price, 'Fuel price', true); const liters = distance * economy / 100;
      return calcResult(liters * price, 'trip cost', `${distance}*${economy}/100*${price}`, [{ label: 'Fuel needed', value: liters, suffix: ' L' }, { label: 'Cost per kilometer', value: distance === 0 ? 0 : liters * price / distance, prefix: '$' }]);
    }),
    calculatorTool('paint', 'Paint coverage calculator', 'Estimate paint volume from wall area, coverage, and coats.', 'home', [
      numberField('area', 'Paintable area', 60, 'm²', { min: 0 }), numberField('coverage', 'Coverage', 10, 'm²/L', { min: 0 }), numberField('coats', 'Coats', 2, '', { min: 1, step: 1 }),
    ], (v) => {
      const area = positive(v.area, 'Area', true); const coverage = positive(v.coverage, 'Coverage'); const coats = Math.max(1, Math.round(positive(v.coats, 'Coats'))); const liters = area * coats / coverage;
      return calcResult(liters, 'liters of paint', `${area}*${coats}/${coverage}`, [{ label: 'Recommended with 10% buffer', value: liters * 1.1, suffix: ' L' }, { label: 'Coated area', value: area * coats, suffix: ' m²' }]);
    }),
    ...
    calculatorTool('mileage', 'Mileage reimbursement calculator', 'Calculate a mileage reimbursement from distance and rate.', 'home', [
      numberField('distance', 'Business distance', 240, 'miles', { min: 0 }), numberField('rate', 'Rate per mile', 0.7, '$', { min: 0 }),
    ], (v) => {
      const distance = positive(v.distance, 'Distance', true); const rate = positive(v.rate, 'Rate', true);
      return calcResult(distance * rate, 'reimbursement', `${distance}*${rate}`, [{ label: 'Distance', value: distance, suffix: ' mi' }, { label: 'Rate', value: rate, prefix: '$', suffix: '/mi' }], 'Enter the rate that applies to your organization and jurisdiction.');
    }),
    calculatorTool('work-time', 'Work time calculator', 'Find elapsed paid time after an unpaid break.', 'clock', [
      numberField('start', 'Start time', 8.5, '24h decimal', { min: 0, max: 24, help: '8.5 means 8:30.' }), numberField('end', 'End time', 17.25, '24h decimal', { min: 0, max: 48 }), numberField('break', 'Unpaid break', 30, 'minutes', { min: 0 }),
    ], (v) => {
      const start = finiteNumber(v.start, 'Start time'); let end = finiteNumber(v.end, 'End time'); const breakMinutes = positive(v.break, 'Break', true); if (end < start) end += 24;
    ...
    calculatorTool('date-difference', 'Date difference calculator', 'Count exact elapsed calendar days between two dates.', 'clock', [
      ...
      return calcResult(days, 'days', `${Math.abs(end - start)}/86400000`, [{ label: 'Weeks', value: days / 7 }, { label: 'Approximate years', value: days / 365.2425 }, { label: 'Direction', value: end >= start ? 'Forward' : 'Reverse' }], 'Day count uses UTC midnights; years are shown as a 365.2425-day approximation.');
    }),
    calculatorTool('fuel-mix', 'Two-stroke fuel mix', 'Calculate oil volume for a fuel-to-oil ratio.', 'home', [
      numberField('fuel', 'Fuel volume', 5, 'L', { min: 0 }), numberField('ratio', 'Fuel ratio', 50, ':1', { min: 0 }),
    ], (v) => {
      const fuel = positive(v.fuel, 'Fuel volume', true); const ratio = positive(v.ratio, 'Ratio'); const oilLiters = fuel / ratio;
  ...
  const DIGITAL_TOOLS = [
    calculatorTool('number-base', 'Number base converter', 'Convert integers between binary, octal, decimal, and hexadecimal.', 'code', [
      ...
      return calcResult(converted, `base ${to}`, '', [{ label: 'Decimal value', value: parsed }, { label: 'Binary', value: parsed.toString(2) }, { label: 'Hexadecimal', value: parsed.toString(16).toUpperCase() }]);
    }),
  ...
  function fieldValue(field, raw) {
    if (field.type === 'number') return raw === '' ? NaN : Number(raw);
    return raw;
  }

  function detailValue(detail, settings) {
    const value = typeof detail.value === 'number' ? formatNumber(detail.value, settings) : String(detail.value ?? '');
  ...
  function initialState() {
    ...
    const settings = {
      ...DEFAULT_SETTINGS,
      ...savedSettings,
      precision: [4, 6, 8, 10, 12].includes(precision) ? precision : DEFAULT_SETTINGS.precision,
    };
    ...
    return {
      ...
      currentTool: null,
      openCategories: new Set(['conversions']),
      drawerOpen: false,
      toastTimer: null,
    };
  }
  ...
  function commitExpression(calculation) {
    ...
    if (!latest || latest.expression !== expression || latest.result !== calculation.value) {
      state.history.unshift({ expression, result: calculation.value, timestamp: Date.now() });
  ...
  function saveSettings() {
    ...
    applyTheme();
    dom.angleBadge.textContent = state.settings.angle.toUpperCase();
    updateExpression();
  ...
  function syncSettingsControls() {
    ...
    dom.angleSetting.value = state.settings.angle;
  ...
  function setMode(mode) {
    ...
    const toggle = document.querySelector('[data-action="scientific-toggle"]');
    if (toggle) toggle.textContent = state.mode === 'scientific' ? 'Hide scientific keys' : 'Show scientific keys';
  }
  ...
  function openDrawer() {
    ...
    document.querySelectorAll('[data-action="tools"], #drawerHandle').forEach((button) => button.setAttribute('aria-expanded', 'true'));
  ...
  function closeDrawer(options) {
    ...
    document.querySelectorAll('[data-action="tools"], #drawerHandle').forEach((button) => button.setAttribute('aria-expanded', 'false'));
    ...
    const markup = TOOL_CATEGORIES.map((category) => {
      ...
      return `<section class="category-block${open ? ' is-open' : ''}" data-category="${category.id}">
        ...
        <div class="category-items"${open ? '' : ' hidden'}>
          ${tools.map((tool) => `<button class="tool-item" type="button" data-tool-id="${tool.id}"><span>${escapeHtml(tool.shortTitle)}</span>${tool.kind === 'unit' ? '<small>Convert</small>' : ''}</button>`).join('')}
        </div>
      </section>`;
    }).join('');
  ...
  function renderQuickTools() {
    const ids = ['unit-length', 'calc-loan-payment', 'calc-bmi', 'calc-date-difference'];
    dom.quickGrid.innerHTML = ids.map((id) => {
      const tool = TOOL_MAP.get(id); const category = CATEGORY_BY_TOOL.get(id);
      return `<button class="quick-card" type="button" data-tool-id="${tool.id}">
        <span class="tool-icon">${iconMarkup(tool.icon)}</span>
        <strong>${escapeHtml(tool.shortTitle)}</strong>
        <span>${escapeHtml(category.title)} · ${escapeHtml(tool.description)}</span>
      </button>`;
    }).join('');
  }

  function unitOptions(tool, selected) {
    return tool.units.map((item, index) => `<option value="${index}"${index === Number(selected) ? ' selected' : ''}>${escapeHtml(item.name)} (${escapeHtml(item.symbol)})</option>`).join('');
  }
    ...
    return `<div class="converter-layout">
      <div class="tool-form-card">
        ...
        <div class="field-stack">
          <label class="field-card"><span>From</span><span class="input-shell"><input id="unitInput" data-unit-value type="number" inputmode="decimal" value="1" step="any" aria-label="Value to convert"><select id="unitFrom" data-unit-from aria-label="From unit">${unitOptions(tool, tool.defaultFrom)}</select></span></label>
          ...
          <label class="field-card"><span>To</span><span class="input-shell"><input id="unitOutput" data-unit-output type="text" value="" readonly aria-label="Converted value"><select id="unitTo" data-unit-to aria-label="To unit">${unitOptions(tool, tool.defaultTo)}</select></span></label>
        </div>
        <p class="tool-error" id="toolError"></p>
        <div class="tool-actions"><button class="secondary-button" type="button" data-tool-action="reset">Reset</button><button class="primary-button" type="button" data-tool-action="copy-tool">${iconMarkup('copy')} Copy result</button></div>
  ...
  function fieldMarkup(field) {
    ...
    if (field.type === 'select') {
      ...
      control = `<span class="input-shell"><input id="field-${field.id}" name="${field.id}" data-calc-field type="number" inputmode="decimal" value="${escapeHtml(field.default)}" step="${escapeHtml(field.step)}"${field.min !== undefined ? ` min="${field.min}"` : ''}${field.max !== undefined ? ` max="${field.max}"` : ''} placeholder="${escapeHtml(field.placeholder)}"><span style="padding:0 .85rem;color:var(--ink-3);font-size:.72rem;white-space:nowrap">${escapeHtml(field.unit)}</span></span>`;
    } else {
      const type = field.type === 'date' ? 'date' : field.type === 'number' ? 'number' : 'text';
      const inputMode = field.type === 'number' ? ' inputmode="decimal"' : '';
      control = `<input class="smart-input" id="field-${field.id}" name="${field.id}" data-calc-field type="${type}"${inputMode} value="${escapeHtml(field.default)}"${field.step ? ` step="${escapeHtml(field.step)}"` : ''}${field.min !== undefined ? ` min="${field.min}"` : ''}${field.max !== undefined ? ` max="${field.max}"` : ''} placeholder="${escapeHtml(field.placeholder || '')}">`;
    }
    return `<label class="${classes}" for="field-${field.id}"><span>${escapeHtml(field.label)}</span>${control}${field.help ? `<span class="field-help">${escapeHtml(field.help)}</span>` : ''}</label>`;
  }
  ...
  function renderResultCard(result, tool) {
    const primary = typeof result.primary === 'number' ? formatNumber(result.primary, state.settings) : String(result.primary);
    return `<span class="result-kicker">Live result</span>
  ...
  function calculateCurrentUnitTool(tool) {
    ...
    try {
      ...
      output.value = formatNumber(result.value, state.settings, { separators: false });
  ...
  function calculateCurrentCalculatorTool(tool) {
    ...
    try {
      ...
      const primary = typeof result.primary === 'number' ? formatNumber(result.primary, state.settings) : result.primary;
  ...
  function resetCurrentTool() {
    ...
    if (!tool) return;
    if (tool.kind === 'unit') {
      document.querySelector('[data-unit-value]').value = '1';
      ...
      tool.fields.forEach((field) => { const element = document.getElementById(`field-${field.id}`); if (element) element.value = String(field.default); });
    }
    refreshCurrentToolResult();
  }
  ...
  function bindToolEvents() {
    dom.toolWorkspace.querySelectorAll('input, select').forEach((element) => {
      element.addEventListener('input', refreshCurrentToolResult);
      element.addEventListener('change', refreshCurrentToolResult);
    });
  }
  ...
  function renderHistory() {
    ...
    dom.historyList.innerHTML = state.history.map((item, index) => `<article class="history-item"><button type="button" data-history-index="${index}"><span class="history-expression">${escapeHtml(item.expression)}</span><strong class="history-result">${escapeHtml(formatNumber(item.result, state.settings))}</strong></button><time datetime="${new Date(item.timestamp).toISOString()}">${escapeHtml(new Intl.DateTimeFormat(undefined, { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' }).format(item.timestamp))}</time></article>`).join('');
  }
  ...
  function handleAction(action, source) {
    switch (action) {
      ...
      case 'scientific-toggle': setMode(state.mode === 'scientific' ? 'basic' : 'scientific'); break;
      case 'copy-result':
  ...
  function cacheDom() {
    dom = {
      ...
      angleBadge: document.getElementById('angleBadge'), scientificKeys: document.getElementById('scientificKeys'), appMenu: document.getElementById('appMenu'), menuButton: document.getElementById('menuButton'),
      ...
      quickGrid: document.getElementById('quickGrid'), toolDrawer: document.getElementById('toolDrawer'), drawerBackdrop: document.getElementById('drawerBackdrop'), toolSearch: document.getElementById('toolSearch'), toolCategories: document.getElementById('toolCategories'),
      settingsDialog: document.getElementById('settingsDialog'), angleSetting: document.getElementById('angleSetting'), precisionSetting: document.getElementById('precisionSetting'), separatorSetting: document.getElementById('separatorSetting'), hapticSetting: document.getElementById('hapticSetting'),
  ...
  function bindGlobalEvents() {
    document.addEventListener('click', (event) => {
      ...
      if (toolButton) { openTool(toolButton.dataset.toolId); return; }
      ...
      if (historyButton) {
        ...
        if (item) { dom.historyDialog.close(); showCalculatorHome(); setExpression(item.expression); }
        return;
      }
    ...
    dom.angleSetting.addEventListener('change', () => { state.settings.angle = dom.angleSetting.value; saveSettings(); });
