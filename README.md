...
A responsive, dependency-free HTML build of The Everything Calculator 1.0.8. It includes a live scientific expression engine, native MathML formula layout, 34 conversion families, 58 practical calculators, light/dark/system themes, nested favorite folders, local history, bundled icons, live reference-rate currency conversion, visual graphs/shapes, and offline PWA support for the local app shell.

## Version 1.0.8 highlights

- Folder Rename and Move controls are anchored to the top edge, and folder creation, renaming, and deletion use app-themed dialogs
- The main calculator commit key is labeled Save Calculation
- Triangle side and angle values stay outside the diagram and clear of the reference square
- Cylinder and cone front base edges use the same ellipse perspective as their top faces
- Quadratic results include the apex X and Y coordinates
