...
assert.equal(calculator.VERSION, '1.0.9');
assert.equal(calculator.TOOL_CATEGORIES.length, 11);
...
assert.equal(new Set(catalogIds).size, 92);
...
assert.match(index, /<small class="version-label">v1\.0\.9<\/small>/);
...
assert.match(worker, /everything-calculator-v1\.0\.9/);
