...
assert.equal(calculator.ALL_TOOLS.length, 102);
assert.equal(calculator.VERSION, '1.0.10');
...
assert.equal(new Set(catalogIds).size, 102);
...
assert.match(index, /<small class="version-label">v1\.0\.10<\/small>/);
...
assert.match(worker, /everything-calculator-v1\.0\.10/);
