...
assert.equal(calculator.VERSION, '1.0.4');
...
assert.match(index, /<small class="version-label">v1\.0\.4<\/small>/);
...
assert.match(worker, /everything-calculator-v1\.0\.4/);
assert.match(index, /data-key="backspace"[\s\S]*data-key="\/"/);
...
assert.match(appSource, /'calc-rectangle':\s*'l×w×h'/);
assert.match(appSource, /'calc-rectangular-prism':\s*'V=l×w×h'/);
assert.match(appSource, /LIVE GRAPH · 0\.2 subdivisions/);
assert.match(appSource, /data-reference-scale/);
